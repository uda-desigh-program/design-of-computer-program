...
def word_plays(hand, board_letters):
    "Find all word plays from hand that can be made to about with a letter on board."
...
def longest_words(hand, board_letters):
    ...
    all_possible_words = word_plays(hand, board_letters)
    return sorted(all_possible_words, reverse=True,key=len)
...
def topn(hand, board_letters, n=10):
    ...
    all_possible_words = word_plays(hand, board_letters)
    return sorted(all_possible_words, reverse=True, key=word_score)[:n]
