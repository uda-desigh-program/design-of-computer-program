...
def boggle_words(board, minlength=3):
    ...
    for (i, sq) in enumerate(board):
        if sq != BORDER:
            results |= (search_word('', [i], board, N, minlength))
    return results


def search_word(prefix, path, board, N, minlength):
    results = set()
    if prefix in WORDS and len(prefix)>=minlength: results.add(prefix)
    if prefix in PREFIXES:
        for nebr in neighbors(path[-1], N):
            if nebr not in path and board[nebr] != BORDER:
                results |= (search_word(prefix+board[nebr], path+[nebr], board, N, minlength))
    return results
