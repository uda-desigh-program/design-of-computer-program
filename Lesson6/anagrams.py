...
def find_anagrams(letters, pre_word, shortest):
    results = set()
    for w in find_words(letters):
        if len(w) >= shortest and w > pre_word:
...
#
#     results = set()
#     all_words = [w for w in find_words(phrase) if len(w) >= shortest]
#     if len(all_words) == 0: return all_words
#     # print all_words
#
#     for now_word in all_words:
#         left_words = anagrams(removed(phrase, now_word), shortest)
#         if left_words == None:
#             results.add(now_word)
#         elif len(left_words) != 0:
