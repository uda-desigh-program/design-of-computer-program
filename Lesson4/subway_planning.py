...
def subway(**lines):
    ...
    all_stops = collections.defaultdict(dict)

    def add_neighbor(stop1, stop2, line):
        all_stops[stop1][stop2] = line

    for (line, stops) in lines.items():
        stops = stops.split()
        for now in range(len(stops)-1):
            all_stops[stops[now]][stops[now+1]] = line
            all_stops[stops[now+1]][stops[now]] = line
    return all_stops
...
def ride(here, there, system=boston):
    ...
    return shortest_path_search(here, lambda stop: system[stop], lambda now_stop: there == now_stop)
